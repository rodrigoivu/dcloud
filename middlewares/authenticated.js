'use strict'
 ...
 var secret = 'clave_secreta_desimat_cloud';
...
//================================================
 ...
 exports.ensureAdmin = function(req, res, next){
 	...
 	if ( usuario.role === 'ADMIN_DESIMAT' || usuario.role === 'ADMIN_ROLE' ){
 		next();
 		return;
 	} else {
...
//================================================
 ...
 exports.ensureAdminUser = function(req, res, next){
 	...
 	var usuario = req.user; // viene del decode del token
 	if ( usuario.role === 'ADMIN_DESIMAT' || usuario.role === 'ADMIN_ROLE' || usuario.role === 'USER_AVANZADO'  ){
 		next();
 		return;
 	} else {
 ...
 exports.ensureAdminMedical = function(req, res, next){
 	...
 	if ( usuario.role === 'ADMIN_ROLE' || usuario.role === 'MEDICAL_ROLE' ){
 		...
 		return;
  		...
  		return res.status(404).send({message:'Token no válido - No es administrador - No está autorizado' });
 	}
 }
 //================================================
